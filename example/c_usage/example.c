...
static void test_ping_message(void) {
    ...
    h6xserial_msg_ping_t ping_msg = {0};
    ping_msg.value = 42;

    uint8_t buffer[256];
    size_t encoded_len = h6xserial_msg_ping_encode(&ping_msg, buffer, sizeof(buffer));
    ...
    h6xserial_msg_ping_t decoded_ping = {0};
    bool decode_ok = h6xserial_msg_ping_decode(&decoded_ping, buffer, encoded_len);
...
static void test_temperature_message(void) {
    ...
    h6xserial_msg_temperature_t temp_msg = {0};
    temp_msg.value = 23.5f;

    uint8_t buffer[256];
    size_t encoded_len = h6xserial_msg_temperature_encode(&temp_msg, buffer, sizeof(buffer));

    TEST_ASSERT(encoded_len == 4, "Temperature encode returns 4 bytes");
    print_hex("Encoded temperature", buffer, encoded_len);

    h6xserial_msg_temperature_t decoded_temp = {0};
    bool decode_ok = h6xserial_msg_temperature_decode(&decoded_temp, buffer, encoded_len);
...
static void test_firmware_version_message(void) {
    ...
    h6xserial_msg_firmware_version_t fw_msg = {0};
    const char* version_string = "v1.2.3-beta";
    fw_msg.length = strlen(version_string);
    memcpy(fw_msg.data, version_string, fw_msg.length);

    uint8_t buffer[256];
    size_t encoded_len = h6xserial_msg_firmware_version_encode(&fw_msg, buffer, sizeof(buffer));
    ...
    bool decode_ok = h6xserial_msg_firmware_version_decode(&decoded_fw, buffer, encoded_len);
...
static void test_multi_temperature_message(void) {
    ...
    h6xserial_msg_multi_temperature_t multi_temp = {0};
    multi_temp.length = 4;
    ...
    uint8_t buffer[256];
    size_t encoded_len = h6xserial_msg_multi_temperature_encode(&multi_temp, buffer, sizeof(buffer));
    ...
    h6xserial_msg_multi_temperature_t decoded_multi = {0};
    bool decode_ok = h6xserial_msg_multi_temperature_decode(&decoded_multi, buffer, encoded_len);
...
static void test_sensor_data_message(void) {
    ...
    bool decode_ok = h6xserial_msg_sensor_data_decode(&decoded_sensor, buffer, encoded_len);
...
static void test_led_control_message(void) {
    ...
    h6xserial_msg_led_control_t led = {0};
    led.led_id = 1;
    led.red = 255;
    led.green = 128;
    led.blue = 64;
    ...
    uint8_t buffer[256];
    size_t encoded_len = h6xserial_msg_led_control_encode(&led, buffer, sizeof(buffer));

    TEST_ASSERT(encoded_len == 5, "LED control encode returns 5 bytes");
    print_hex("Encoded LED control", buffer, encoded_len);

    h6xserial_msg_led_control_t decoded_led = {0};
    bool decode_ok = h6xserial_msg_led_control_decode(&decoded_led, buffer, encoded_len);
...
static void test_motor_speeds_message(void) {
    ...
    h6xserial_msg_motor_speeds_t motors = {0};
    motors.length = 4;
    motors.data[0] = 1000;
    motors.data[1] = -500;
    motors.data[2] = 750;
    ...
    uint8_t buffer[256];
    size_t encoded_len = h6xserial_msg_motor_speeds_encode(&motors, buffer, sizeof(buffer));

    TEST_ASSERT(encoded_len == 8, "Motor speeds encode returns 8 bytes (4 int16)");
    print_hex("Encoded motor speeds", buffer, encoded_len);

    h6xserial_msg_motor_speeds_t decoded_motors = {0};
    bool decode_ok = h6xserial_msg_motor_speeds_decode(&decoded_motors, buffer, encoded_len);
